...
st.set_page_config(page_title="Playtest App V2.2", layout="wide", page_icon=CCUG_LOGO_URL)
...
def load_links():
    try:
        # Correction pour lire le CSV avec ';' et nettoyer les noms de colonnes
...
# 7. TABS & LAYOUT
# Création d'une ligne de colonnes pour mettre le bouton à gauche des onglets
col_disc, col_tabs = st.columns([0.15, 0.85])

with col_disc:
    # Bouton Discord aligné à gauche
    if not df_links.empty:
        # Recherche robuste de la classe dans le CSV (nettoyage des espaces et casse)
